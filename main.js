...
const arpInds = [[0], [1], [2], [3], [0, 1], [0, 2], [0, 3], [1, 2], [1, 3], [2, 3], [0, 1, 2], [0, 1, 3], [0, 2, 3], [1, 2, 3]];
...
function displayFirst(chord) {
    ...
    //var newString = oldStringBass + " <" + chord.toString() + "> " + keyMap[chord[0]] + chordMapForDisplay[chord[1]][chord[2]]; 
    //bassDisplay.innerHTML = newString;
...
function displayStep(chord, t, m, s) {
    ...
    for (var i = 0; i < m.length; i++) {
        oldStringTreble += "(" + m[i][0].toString() + ")" + m[i][1].toString() + " ";
    }
    */
    /*
    oldStringBass += "  ";
    ...
    for (var i = 0; i < s.length; i++) {
        oldStringBass += "(" + s[i][0].toString() + ") ";
        for (var j = 1; j < s[i].length; j++) {
            oldStringBass += s[i][j].toString() + " ";
        }
        oldStringBass += " | ";
    }
    */
    //oldStringBass += "<br\>[" + t.toString() + "]--> <" + chord.toString() + "> " + keyMap[chord[0]] + chordMapForDisplay[chord[1]][chord[2]];
